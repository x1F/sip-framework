...
public class MarkdownUtil {
    ...
    private static final String MARKDOWN_IMAGE_REGEX =
            "!\\[(?<alttext>[^\\]]*)\\]" +             // Captures alt text
            "\\(" +                                    // Opening parenthesis
            "(?<uri>[^\\\"\\)]+?)" +                   // Captures uri (up to " or ))
            "(?:\\s*\"(?<optionalpart>[^\"]*?)\")?" +  // Captures optional title
            "\\)";                                     // Closing parenthesis

    private static final Pattern MARKDOWN_IMAGE_PATTERN = Pattern.compile(MARKDOWN_IMAGE_REGEX);

    public String readMarkdownFileAndEmbedLocalImages(final ClassPathResource markdownResource) throws IOException {
        ...
        while (matcher.find()) {
            ...
            if (imageUri.isAbsolute()) {
                log.debug("Skipping embedding of absolute/remote image at path '{}' for markdown-file at {}", imageUri, markdownResource);
                appendOriginalMatch(matcher, markdownWithEmbedded);
            } else {
                final var imagePath = Path.of(imageUri.getPath());
                final var imageResource = imagePath.isAbsolute() ? new FileSystemResource(imagePath) : markdownResource.createRelative(imagePath.toString());
                if (!imageResource.isReadable()) {
                    log.warn("Won't embed image at path '{}' for markdown-file at {}: Resource is not readable", imageResource, markdownResource);
                    appendOriginalMatch(matcher, markdownWithEmbedded);
                } else {
                    final var altText = matcher.group("alttext");
                    final var mime = Optional.ofNullable(imageResource.getURL().openConnection().getContentType()).orElse("application/octet-stream");
                    final var base64Tag = String.format("![%s](data:%s;base64,%s)", altText, mime, Base64.getEncoder().encodeToString(imageResource.getContentAsByteArray()));
